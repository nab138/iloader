...
use apple_codesign::{
    ...
    SignedMachOInfo, SigningSettings, UnifiedSigner,
...
fn signing_material(
    cert_identity: &CertificateIdentity,
) -> Result<(InMemoryPrivateKey, CapturedX509Certificate), Report> {
    let private_key_der = cert_identity.private_key.to_pkcs8_der()?;
    let private_key = InMemoryPrivateKey::from_pkcs8_der(private_key_der.as_bytes())?;
    let certificate = CapturedX509Certificate::from_der(cert_identity.certificate.to_der()?)?;

    Ok((private_key, certificate))
}

fn signing_settings<'a>(
    private_key: &'a InMemoryPrivateKey,
...
fn apply_nested_app_extension_seals(
    ...
    let (private_key, certificate) = signing_material(cert_identity)?;
    let mut settings = signing_settings(&private_key, certificate);
    settings
