...
impl CertificateIdentity {
    ...
    pub async fn retrieve(
        ...
        // ponytail: Generate a fresh keypair when no valid certificate matched the existing key
        let pr = {
        ...
        let email_hash = hex::encode(sha2::Sha256::digest(apple_email.as_bytes()));
        let _ = storage.store_data(
            &format!("{}/key", email_hash),
            pr.to_pkcs8_der()?.as_bytes(),
        );
        ...
        for cert in developer_session
            ...
            .filter(|c| {
                c.cert_content.is_some()
                    ...
                    && c.status_code.unwrap_or(0) == 0
                    && !c.status.as_deref().unwrap_or("").eq_ignore_ascii_case("revoked")
                    && !c.status.as_deref().unwrap_or("").eq_ignore_ascii_case("expired")
            })
        {
            let x509_cert =
                Certificate::from_der(cert.cert_content.as_ref().ok_or_report()?.as_ref())?;

            // ponytail: Reject certificates whose validity period has expired
            if x509_cert.tbs_certificate.validity.not_after.to_unix_duration() <= now_unix {
                tracing::warn!(
                    "Skipping expired certificate with serial: {:?}",
                    cert.serial_number
                );
                continue;
            }
                            ...
                            if existing_certs.is_none() {
                                existing_certs = Some(
                                    developer_session
                                        ...
                                        .iter()
                                        .filter(|c| {
                                            c.serial_number.is_some()
                                                && !c
                                                    .status
                                                    .as_deref()
                                                    .unwrap_or("")
                                                    .eq_ignore_ascii_case("revoked")
                                        })
                                        .cloned()
                                        .collect(),
                                );
